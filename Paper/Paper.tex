...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\node[c] (x1) at (1.65,1.5) {\small $\times$};
		\node[c] (x2) at (3.05,1.5) {\small $\times$};
		\node[c] (xT) at (6.35,1.5) {\small $\times$};

		\draw (0,-1.5) edge[dashed] (h1);	
		\draw (2.5,-1.5) edge[dashed] (h2);	
		\draw (8,-1.5) edge[dashed] (hT);	
		...
		\draw (-4,4) edge[dashed] (Nx1);
		\draw (-4,3.25) edge[dashed] (Nx2);
		\draw (-4,2.5) edge[dashed] (Nx3);
	...
	\end{tikzpicture}
	\caption[]
	{\small \textit{}\label{fig:2} General case of an attentional layer. Given some sequence $[\pmb{h}_1,\dots,\pmb{h}_T]$ (e.g. the hidden output of an RNN), we may use an neural function $a$ to obtain a weighting vector $\pmb{a} \in \mathbb{R}^{T}$. Multiplying each sequence element by its corresponding weight and summing up the results, we end up with a context vector $\pmb{c}$. Note that when $a$ accepts an external vector (or vector sequence) as its argument, the result is a context-dynamic selection of the $[\pmb{h}_1,\dots]$ sequence.}
\end{figure}
