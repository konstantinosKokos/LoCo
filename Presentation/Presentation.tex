...
\begin{frame}{Introduction: RNNs}
	...
	\small $NN: \pmb{x}_{t_i} \mapsto \pmb{y}_{t_i}$\\
	\alert{No memory}
	\end{minipage}
	\pause
	\hspace{1cm}
	...
	\small{ $RNN: \pmb{x}_{t_0} \otimes \pmb{x}_{t_1} \otimes \dots \otimes \pmb{x}_{t_i} \mapsto \pmb{y}_{t_i}$}\\
	\alert{Finite memory}
	\end{minipage}
	\pause
...
\begin{frame}{Controller: Signal-Flow (1/2)}
...
\begin{frame}{Controller: Signal-Flow (2/2)}
	...
	\tikzset{split/.style={circle, draw=black,inner sep=0pt, minimum size = 3mm, fill=white}}
		...
		\draw[->,thick] (LL) edge (sL);		
		\draw[->,thick] (sL) edge (10, -5);
		% labels
		\node[label=above right :$\scriptsize\textcolor{white}{\xx_t}$] (xt) at (0,0) {};
		...
		\node[label=left :$\scriptsize\textcolor{white}{[\hh_t^1;\hh_t^2;\dots;\hh_t^L]}$] (xt) at (10,-5) {};
	\end{tikzpicture}
	\end{figure}
\end{frame}
...
\begin{frame}{Controller: Outputs}
	...
	\begin{itemize}
		...
		\item Free gates %$\pmb{f}_t^i \in [0,1]$		
...
\begin{frame}{Memory Adressing: Content-Lookup}
	...
	\pause
	\alert{Weightings} $w$ given by $\mathcal{C}$
	\[
	\mathcal{C}(M,\pmb{k},\beta)[i]= 
	\frac {exp \{ \mathcal{D} (\pmb{k},M[i,:])\beta \} }
	{ \sum_{j}{exp \{ \mathcal{D} (\pmb{k},M[j,:])\beta \}}}
	\]
	...
	\[
	M_t = M_{t-1} \circ (\pmb{1} - \pmb{w}_t^w\pmb{e}_t^T) + \pmb{w}_t^w\pmb{v}_t^T
	\]
\end{frame}
