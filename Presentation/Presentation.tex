...
\begin{frame}{Controller: Full View}
	...
	\tikzset{split/.style={circle, draw=black,inner sep=0pt, minimum size = 3mm, fill=white}}
		...
		\draw (L3) edge[->] (Ld);
		\draw (Ld) edge[->] (LL);
		\draw[->] (x1) edge (L1.north);
		\draw[->] (x2) edge (L2.north);
		\draw[->] (x3) edge (L3.north);
		\draw[->] (xL) edge (LL.north);
		\draw[->] (xd) edge (Ld.north);
		\draw[->] (1,0) |- (s0);
		\draw[->] (h1) edge (s0);
		\draw[->] (s0) edge (s1);
		\draw[->] (h2) edge (s1);
		...
		\node[label=below :$\scriptsize\textcolor{white}{[\xx_t;\hh_t^1]}$] (xt) at (4,-3) {};
		\node[label=below :$\scriptsize\textcolor{white}{[\xx_t;\hh_t^1;\hh_t^2]}$] (xt) at (6,-3) {};
		\node[label=left :$\scriptsize\textcolor{white}{[\xx_t;\hh_t^1;\dots;\hh_t^L]}$] (xt) at (10,-5) {};
	\end{tikzpicture}
	\end{figure}
\end{frame}
